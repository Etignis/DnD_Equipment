...
window.onload = function(){
	...
	function createComboBox(src, param) {
		...
		if(Array.isArray(src)) {
			...
			for(key in src) {
				if(sItemsList && sItemsList.indexOf(key)>-1 || !sItemsList){
					var type = src[key];
	...
	function getItemAttrFromDB(oDataSource, oParam) {
		function replace(oSrc, sAttr){
			for (key in oSrc) {
				var oRe = new RegExp(key);
				sAttr = sAttr.replace(oRe, oSrc[key].text[sLang].title)
			}
			return sAttr;
		}	
    ...
    for(var i=0; i<aParam.length; i++) {
      ...
      if(oDataSource[sAttr]){
				//aReturn.push(find(oDataSource, sAttr).join("_"));
				/**/
        ...
        for(var i=0; i<aLang.length; i++){
          if(oDataSource[sAttr].text[aLang[i]]) {
            ...
            break;
				...
				// search in text
				sAttr = replace(oDataSource, sAttr);
				aReturn.push(sAttr);
			}
    }
	...
	function createCard(oItem, lang, sLockedItem, sView) {
      ...
      // var s_typeAdditions = getItemAttr(oItem, "typeAdditions", lang);
			var s_typeAdditions = getItemAttrFromDB(oTypeAdditions, {attr: getItemAttr(oItem, "typeAdditions", lang), subattr: s_gender, lang: lang});
      ...
      var fText = (sView == 'text');
			...
			// armor AC
			var sAC = getItemAttrFromDB(oAC, {attr: getItemAttr(oItem, "ac", lang), lang: lang});
			if(sAC) {
				sAC = "<div class='ac'>Защита: "+sAC+"</div>";
			}
			
			// stealth disadvantage
			var s_stealthDis = "";
			if (oItem.en.stealthDis == true){
				s_stealthDis = (lang == "ru")? "Помеха на Скрытность":"Disadvantage on Stealth";
			}
        ...
        s_weight = "<div class='weight'>вес: "+s_weight+" "+((lang == "ru")? "фнт.":"lb.")+"</div>";
			...
			// weapon props
			var a_props = getItemAttr(oItem, "props", lang);
			if(a_props){
				a_props = a_props.map(function(sProp){
					return getItemAttrFromDB(oWeaponProps, {attr: sProp, lang: lang});
				});
			}
			var s_props="";
			if(a_props.length>0) {
				s_props = "<div class='props'>" + a_props.join(", ")+"</div>";
			}
			
			var s_damage=getItemAttr(oItem, "damageVal", lang);
			if(s_damage) {
				s_damage = "<div class='damage'>" + s_damage + " " + getItemAttrFromDB(oDamageType, {attr: getItemAttr(oItem, "damageType", lang), lang: lang})+"</div>";
			}
      ...
      //s_text = fText? s_text.split("<br>").map(item => "<p>"+item+"</p>").join(""): s_text;
            ...
            "<div class='header_info'>"+
              ...
              '<div class="subtitle">' + s_type+  " " + s_typeAdditions + '</div>'+
            "</div>"+
	...
	function showFiltered(oParams) {
      ...
      var fRandom = oParams.fRandom;
		...
		if(aProps && aProps.length>0) {
			filteredItems = filteredItems.filter(function(Item){
				for(var i = 0;Item.en.props && i < aProps.length; i++) {
					for(var j=0; j<Item.en.props.length; j++) {
						if (Item.en.props[j].toLowerCase().trim().indexOf(aProps[i].toLowerCase().trim()) > -1) {
							return true;
						}
					}
	...
	function createPropsCombobox(bShow, isOpen) {
		if(isOpen == undefined)
			isOpen = false;
		if(bShow) {
			if (!$("#PropsCombobox").length){
				var s1=createComboBox(oWeaponProps, {id: "PropsCombobox", title: "Свойства оружия", checkAll: false, isOpen: isOpen});
				$("#SourceCombobox").parent().before("<div class='mediaWidth'>" + s1 + "</div>");
			}
		} else if ($("#PropsCombobox").length) {
			$("#PropsCombobox").parent().remove();
		}
	}
	
	function createArmorSubTypeCombobox(bShow, isOpen) {
		if(isOpen == undefined)
			isOpen = false;
		if(bShow){
			if(!$("#ArmorTypeAddCombobox").length) {
				var aSubTypes = oTypes.armor.subTypes;
				var s1=createComboBox(oTypeAdditions, {id: "ArmorTypeAddCombobox", title: "Подтип брони", checkAll: false, isOpen: isOpen, sList: aSubTypes});
				$("#TypeCombobox").parent().after("<div class='mediaWidth'>" + s1 + "</div>");
			}
		} else if ($("#ArmorTypeAddCombobox").length) {
			$("#ArmorTypeAddCombobox").each(function(){
				$(this).parent().remove();
			})
		}
	}
	function createWeaponSubTypeCombobox(bShow, isOpen) {
		if(isOpen == undefined)
			isOpen = false;
		if(bShow){
			if (!$("#WeaponTypeAddCombobox").length){
				var aSubTypes = oTypes.weapon.subTypes;
				var s1=createComboBox(oTypeAdditions, {id: "WeaponTypeAddCombobox", title: "Подтип оружия", checkAll: false, isOpen: isOpen, sList: aSubTypes});
				$("#TypeCombobox").parent().after("<div class='mediaWidth'>" + s1 + "</div>");
			}
		} else if ($("#WeaponTypeAddCombobox").length) {
			$("#WeaponTypeAddCombobox").each(function(){
				$(this).parent().remove();
			})
		}
	}
  ...
  function createAutoSizeTextButton() {
	...
	$("body").on('click', "#TypeCombobox label", function(){
		...
		oTimer = setTimeout(function(){
			updateHash();
			var sTypes = $("#TypeCombobox .combo_box_title").attr("data-val");

			createArmorSubTypeCombobox(sTypes.indexOf("armor")>-1);
			createWeaponSubTypeCombobox(sTypes.indexOf("weapon")>-1);
			createPropsCombobox(sTypes.indexOf("weapon")>-1);
	...
	// AddType combobox
	$("body").on('click', "#TypeAddCombobox label", function(){
		clearTimeout(oTimer);
		oTimer = setTimeout(function(){
			updateHash();
			filterItems();
	...
	$("body").on('click', "#TypeAddCombobox .combo_box_title, #TypeAddCombobox .combo_box_arrow", function(){
	...
	$("body").on('click', "#PropsCombobox .combo_box_title, #PropsCombobox .combo_box_arrow", function(){
		setConfig("PropsOpen", $("#PropsCombobox").attr("data-content-open"));
	});
...
// url filters
	function updateHash() {
		...
		var aArmorAddTypes = $("#ArmorTypeAddCombobox .combo_box_title").attr("data-val");
			if(aArmorAddTypes) aArmorAddTypes = aArmorAddTypes.split(",").map(function(item){return item.trim()});
		var aWeaponAddTypes = $("#WeaponTypeAddCombobox .combo_box_title").attr("data-val");
			if(aWeaponAddTypes) aWeaponAddTypes = aWeaponAddTypes.split(",").map(function(item){return item.trim()});
		var aPropsTypes = $("#PropsTypeCombobox .combo_box_title").attr("data-val");
			if(aPropsTypes) aPropsTypes = aPropsTypes.split(",").map(function(item){return item.trim()});
		// select
		var sLang = $("#LangSelect .label").attr("data-selected-key");
		...
		if(aPropsTypes && aPropsTypes.length>0 && $("#PropsTypeCombobox .combo_box_content input").length > aPropsTypes.length) {
			aFilters.push("props="+aPropsTypes.join(",").replace(/\s+/g, "_"));
		}
  ...
  function getHash(oParams){
    ...
    if(oParams || sHash && !/[^А-Яа-яЁё\w\d\/&\[\]?|,_=-]/.test(sHash)) {
     ...
     // var sRarities = sHash.match(/\brarity=([\w,]+)/);
		  var aArmorAddTypes = sHash.match(/\barmoraddtype=([\w_,-]+)/);
		  var aWeaponAddTypes = sHash.match(/\bweaponaddtype=([\w_,-]+)/);
		  var aPropsTypes = sHash.match(/\bprops=([\w,]+)/);
      ...
      if(sTypes && sTypes[1]) {
      	...
      	$("#TypeCombobox .combo_box_title").attr("data-val", sTypes[1].replace("_", " "))
				...
				createArmorSubTypeCombobox(false);
				createWeaponSubTypeCombobox(false);
				createPropsCombobox(false);
			}
